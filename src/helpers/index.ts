import { addresses, BLOCK_RATE_SECONDS } from "./constants";
...
import type { INFTMetadata, TraitType } from "../pages/Profile/CardNFT";

/**
 * gets marketPrice from Ohm-DAI v2
 * @returns Number like 333.33
 */
// export async function getMarketPrice(provider: any) {
...
export const attributeData = (nftMetaData: INFTMetadata, key: string) =>
  nftMetaData?.attributes.find((attribute: any) => attribute.trait_type === key)?.value || "";
export const getTraitValue = (nft: INFTMetadata, traitType: TraitType): string => {
  if (nft && nft.attributes.length > 0) {
    return nft.attributes.find(attr => attr.trait_type === traitType)?.value as string;
  }
  return "";
};
