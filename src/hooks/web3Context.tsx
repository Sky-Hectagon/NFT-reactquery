...
import { initNetworkFunc, idFromHexString } from "../helpers/NetworkHelper";
...
const defaultChainId = process.env.REACT_ENV === "mainnet" ? 56 : 97;
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  const [connected, setConnected] = useState(false);
  const [address, setAddress] = useState("");
  const [provider, setProvider] = useState<JsonRpcProvider>(NodeHelper.getAnynetStaticProvider(defaultChainId));
  const [networkId, setNetworkId] = useState(defaultChainId);
  const [networkName, setNetworkName] = useState(NETWORKS[defaultChainId].chainName);
  const [providerUri, setProviderUri] = useState(NETWORKS[defaultChainId].uri);
